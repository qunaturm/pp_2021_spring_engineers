...
#include "../../../modules/task_4/vizgalov_a_hoare_sort_simple_merge/hoare_sort_simple_merge.h"
...
void runHoareSort(std::vector<int>* vec, int numThreads) {
    int elementsPerSegment = vec->size() / numThreads + ((vec->size() % numThreads == 0) ? 0 : 1);

    std::vector<std::thread> threads;
    for (int i = 0; i < numThreads; ++i) {
        threads.emplace_back([vec, i, elementsPerSegment]() {
            hoareSort(vec,
